...
namespace ImGui
{
    ...
    IMGUI_API void          PushButtonRepeat(bool repeat);                                  // in 'repeat' mode, ButtonReset*() functions return repeated true in a typematic manner (using io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you can call IsItemActive() after any ButtonReset() to tell if the button is held in the current frame.
