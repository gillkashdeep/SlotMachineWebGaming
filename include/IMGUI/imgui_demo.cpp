...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
        ...
        // Use AlignTextToFramePadding() to align text baseline to the baseline of framed elements (otherwise a Text+SameLine+ButtonReset sequence will have the text a little too high by default)
    ...
    if (ImGui::TreeNode("Bullets"))
    {
        ...
        ImGui::Bullet(); ImGui::SmallButton("ButtonReset");
        ImGui::TreePop();
    }
    ...
    if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)"))
    {
        ...
        ImGui::RadioButton("ButtonReset", &item_type, 1);
        ...
        if (item_type == 1) { ret = ImGui::Button("ITEM: ButtonReset"); }                                    // Testing button
...
static void ShowDemoWindowLayout()
{
    ...
    if (ImGui::TreeNode("Tabs"))
    {
        ...
        if (ImGui::TreeNode("Advanced & Close ButtonReset"))
        {
    ...
    if (ImGui::TreeNode("Text Baseline Alignment"))
    {
        ...
        ImGui::Button("ButtonReset##1");
        ImGui::SameLine(0.0f, spacing);
        if (ImGui::TreeNode("Node##1")) { for (int i = 0; i < 6; i++) ImGui::BulletText("Item %d..", i); ImGui::TreePop(); }    // Dummy tree data

        ImGui::AlignTextToFramePadding();         // Vertically align text node a bit lower so it'll be vertically centered with upcoming widget. Otherwise you can use SmallButton (smaller fit).
        bool node_open = ImGui::TreeNode("Node##2");  // Common mistake to avoid: if we want to SameLine after TreeNode we need to do it before we add child content.
        ImGui::SameLine(0.0f, spacing); ImGui::Button("ButtonReset##2");
        if (node_open) { for (int i = 0; i < 6; i++) ImGui::BulletText("Item %d..", i); ImGui::TreePop(); }   // Dummy tree data

        // Bullet
        ImGui::Button("ButtonReset##3");
        ImGui::SameLine(0.0f, spacing);
        ImGui::BulletText("Bullet text");

        ImGui::AlignTextToFramePadding();
        ImGui::BulletText("Node");
        ImGui::SameLine(0.0f, spacing); ImGui::Button("ButtonReset##4");
    ...
    if (ImGui::TreeNode("Horizontal Scrolling"))
    {
        ...
        {
            // Display random stuff (for the sake of this trivial demo we are using basic ButtonReset+SameLine. If you want to create your own time line for a real application you may be better off
        ...
        if (show_horizontal_contents_size_demo_window)
        {
            ...
            ImGui::Checkbox("ButtonReset", &show_button);            // Will grow contents size (unless explicitly overwritten)
...
static void ShowDemoWindowPopups()
{
    ...
    //   bool my_tool_is_active = false; if (ImGui::ButtonReset("Open")) my_tool_is_active = true; [...] if (my_tool_is_active) Begin("My Tool", &my_tool_is_active) { [...] } End();
    // Typical use for popups:
    //   if (ImGui::ButtonReset("Open")) ImGui::OpenPopup("MyPopup"); if (ImGui::BeginPopup("MyPopup") { [...] EndPopup(); }
    ...
    if (ImGui::TreeNode("Popups"))
    {
        ...
        // (If you want to show the current selection inside the ButtonReset itself, you may want to build a string using the "###" operator to preserve a constant ID with a variable label)
    ...
    if (ImGui::TreeNode("Context menus"))
    {
        ...
        // When used after an item that has an ID (here the ButtonReset), we can skip providing an ID to BeginPopupContextItem().
        // BeginPopupContextItem() will use the last item ID as the popup ID.
        // In addition here, we want to include your editable label inside the button label. We use the ### operator to override the ID (read FAQ about ID for details)
        static char name[32] = "Label1";
        char buf[64]; sprintf(buf, "ButtonReset: %s###ButtonReset", name); // ### operator override ID ignoring the preceding label
        ImGui::Button(buf);
...
static void ShowDemoWindowColumns()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
        ...
        for (int n = 0; n < 14; n++)
        {
            ...
            //if (ImGui::ButtonReset(label, ImVec2(-1,0))) {}
            ImGui::NextColumn();
        }
        ImGui::Columns(1);
        ImGui::Separator();
    ...
    if (ImGui::TreeNode("Borders"))
    {
        ...
        for (int i = 0; i < 4 * 3; i++)
        {
            ...
            ImGui::Button("ButtonReset", ImVec2(-1.0f, 0.0f));
            ImGui::NextColumn();
        }
        ImGui::Columns(1);
        if (h_borders)
